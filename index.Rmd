...
* **Title:** The chart title shows "current cumulative deaths and [predicted cumulative deaths over next 4 weeks]".
...
plot_trends <- function(df_tmp, state, df_pred, pred_cum_death, df_rt=NA) {
  ...
  if (state != 'US') {
    ...
    p_rt = p_rt +
      scale_x_date(limits = c(min_date, max_date)) + 
      scale_y_continuous(label=comma, limits = c(0.6, 1.4)) + 
      theme(axis.text.x = element_blank(),
            plot.margin = unit(c(0, 0, 0, 0), "cm")) + 
      labs(y='', x='') + 
...
# States
```{r, results='asis', fig.height=9, fig.width=7} 
