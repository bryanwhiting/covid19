...
plot_trends <- function(df_tmp, state, df_pred, pred_cum_death) {
  ...
  p <- ggplot() + 
    ...
    annotate("label", x=min_date, y=max(df_tmp$new), label='14-day Lag of New Cases', hjust=0) +
    labs(x='', y='')  
  ...
  p2 <- ggplot() +
    ...
    annotate("label", x=min_date, y=max(df_pred$ul, df_tmp$dead), label='Deaths', hjust=0) + 
    labs(x='', y='')
  ...
  plot_grid(title, plots, ncol=1, rel_heights = c(0.1, .9))
